...
class Server(object):
    ...
    def broadcast(self, message):
        for n in self._neighbours:
            ...
            asyncio.run_coroutine_threadsafe(self.post_message(message, n._port), self._loop)
    ...
    def send_message_response(self, message, port):
        ...
        if (len(n) > 0):
            asyncio.run_coroutine_threadsafe(self.post_message(message, port), self._loop)
