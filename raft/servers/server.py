...
class Server(object):
    ...
    def __init__(self, name, state, log, neighbours, port):
        ...
        thread = Server_Wait_Client(self, self._sock)
        thread.start()
    ...
    def send_message_response(self, message):
        ...
        n = [n for n in self._neighbours if n._name == message.receiver]
        if (len(n) > 0):
            n[0].post_message(message)

    async def post_message(self, message, port):
        # TODO: probably need a serializer
...
class Server_Wait_Client(threading.Thread):
    ...
    def run(self):
        curio.run(self._wait_client_command())

    async def _wait_client_command(self):
        while True:
            data, addr = await self._sock.recvfrom(10000)
            data = data.decode('utf8')
            print('Server at', self._server._port, 'Received data from', addr, data)
            print('Server state from thread is', self._server._state)
            self._server._state.on_client_command(data, addr)
